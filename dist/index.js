...
async function fetchPageMetadata(targetUrl, parsed) {
    try {
        ...
        const extract = (pattern) => pattern.exec(html)?.[1]?.trim() || null;
        const resolveUrl = (u) => { try {
...
function htmlPage(title, content, extraScript = '') {
    return `<!DOCTYPE html>
<html lang="en">
...
app.get('/usage', async (c) => {
    ...
    if (!keyStr)
        return c.json({ error: 'API key required via Authorization header or ?api_key=' }, 401);
